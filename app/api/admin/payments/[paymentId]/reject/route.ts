...
export async function POST(_request: Request, { params }: { params: { paymentId: string } }) {
  ...
  if (!canAccessAdmin(user.email, role)) {
    await auditAdminAccess({
      ...
      path: "/api/admin/payments/reject",
      ok: false
    });
  ...
  const { error: updateError } = await admin.from("payments").update({ status: "rejected" }).eq("id", payment.id).eq("status", "pending");

  if (updateError) {
    return NextResponse.json({ error: updateError.message || "Failed to reject payment." }, { status: 500 });
  }
  ...
  if (payment.user_id) {
    await admin.from("notifications").insert({
      ...
      message: "Your payment could not be verified. Please submit again.",
      type: "payment",
      read: false
    });
  }
  ...
  await auditAdminAccess({
    userId: user.id,
    eventName: "admin_api_reject_payment",
    path: "/api/admin/payments/reject",
    ok: true,
