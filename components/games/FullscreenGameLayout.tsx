...
export function FullscreenGameLayout({ title, canvas, controls, onExit }: FullscreenGameLayoutProps) {
  return (
    <div className="fixed inset-0 z-50 flex min-h-[100dvh] flex-col bg-[#020617]">
      <header className="h-12 flex items-center justify-between px-3 border-b border-cyan-400/20 bg-black/30">
      ...
      <div className="pb-[env(safe-area-inset-bottom)] p-3 grid grid-cols-3 gap-3 bg-black/40 backdrop-blur-xl">{controls}</div>
    </div>
  );
}
